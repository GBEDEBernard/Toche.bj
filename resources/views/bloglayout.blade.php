...
<!-- NAVBAR -->
<nav class="bg-gray-800 text-white p-4 shadow-lg" x-data="{ itineraireOpen: false, mobileOpen: false, profilOpen: false }">
  <div class="flex items-center justify-between w-full">
    ...
    <div class="hidden md:flex items-center gap-8 font-serif uppercase text-sm">
      <a href="{{ route('accueil') }}" class="{{ request()->routeIs('accueil') ? 'text-red-500' : 'hover:text-blue-400' }} transition-colors">Accueil</a>
      <a href="{{ route('site_touristique') }}" class="{{ request()->routeIs('site_touristique') ? 'text-red-500' : 'hover:text-blue-400' }} transition-colors">Sites Touristiques</a>

      <!-- Événements sous-menu -->
      <div class="relative" @mouseenter="itineraireOpen = true" @mouseleave="itineraireOpen = false">
        <button @click="itineraireOpen = !itineraireOpen"
                class="flex uppercase items-center gap-1 transition-colors {{ request()->routeIs('evenements') || request()->routeIs('itineraire.offres') ? 'text-red-500' : 'hover:text-blue-400' }}">
          Événements
        ...
        <div x-show="itineraireOpen" x-transition class="absolute mt-2 w-48 bg-white text-gray-800 rounded-lg shadow-lg z-50">
          <a href="{{ route('evenements') }}" class="block px-4 py-2 hover:bg-gray-100 {{ request()->routeIs('evenements') ? 'text-red-500 font-semibold' : '' }}">Voir tous les événements</a>
          <a href="{{ route('itineraire.offres') }}" class="block px-4 py-2 hover:bg-gray-100 border-t {{ request()->routeIs('itineraire.offres') ? 'text-red-500 font-semibold' : '' }}">Les offres d’itinéraires</a>
        </div>
      </div>

      <a href="{{ route('apropos') }}" class="{{ request()->routeIs('apropos') ? 'text-red-500' : 'hover:text-blue-400' }} transition-colors">À propos</a>
      <a href="{{ route('Contacts') }}" class="{{ request()->routeIs('Contacts') ? 'text-red-500' : 'hover:text-blue-400' }} transition-colors">Contacts</a>
      <a href="{{ route('participer') }}" class="{{ request()->routeIs('participer') ? 'text-red-500' : 'hover:text-blue-400' }} transition-colors">Participer</a>
    </div>
        ...
        <div x-show="profilOpen" @click.away="profilOpen = false" x-transition
          ...
          @else
            ...
            <form method="POST" action="{{ route('logout') }}">
              @csrf
              <button type="submit" class="w-full text-left px-4 py-2 text-gray-700 font-bold hover:bg-red-100 border-t">Déconnexion</button>
            </form>
          @endguest
        </div>
      </div>
...
<footer class="bg-gray-900 text-gray-300 py-10 px-6 md:px-16 lg:px-24">
  <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10 items-start">
    ...
    <div class="flex flex-col items-center md:items-start text-center md:text-left space-y-4">
      <img src="{{ asset('image/logo3.jpg') }}" alt="Logo Toche" class="h-16 w-16 md:h-20 md:w-20 object-contain rounded-full shadow-lg hover:scale-110 transition-transform duration-300"/>
    ...
    <div class="flex flex-col items-center md:items-start text-center md:text-left space-y-4">
      ...
      <p class="text-sm md:text-base leading-relaxed text-gray-400">Godomey Togoudo, Abomey-Calavi, Bénin</p>
      <a href="https://www.google.com/maps/place/TFG+SARL" target="_blank" class="inline-flex items-center justify-center md:justify-start gap-2 mt-2 text-sm md:text-base text-blue-400 hover:text-blue-300 transition-colors">
...
<!-- 🖼️ Modal image responsive -->
<div id="imageModal" class="fixed inset-0 z-50 bg-black bg-opacity-70 hidden items-center justify-center p-4">
    <div class="relative w-full max-w-3xl max-h-full">
        <img id="modalImage" src="" alt="Aperçu image"
...
// Ajoute la fonctionnalité zoom à toutes les images sauf exceptions
document.addEventListener('DOMContentLoaded', () => {
