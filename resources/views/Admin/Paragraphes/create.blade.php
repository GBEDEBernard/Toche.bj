...
@section('content')
<h1 class="text-2xl font-bold m-4">Ajouter un paragraphe à l'evenement : {{ $evenement->nom ?? 'evenement inconnu' }}</h1>

<form action="{{ route('admin.paragraphes.store') }}?evenement_id={{ $evenement->id }}" method="POST" class="p-4">
    @csrf
