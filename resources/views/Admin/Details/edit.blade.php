...
@section('content')
<h1 class="text-2xl font-bold mb-4">Modifier un paragraphe du site : {{ $site->nom ?? 'Site inconnu' }}</h1>

<form action="{{ route('admin.details.update', $detail) }}?site_id={{ $site->id }}" method="POST">
    @csrf
    @method('PUT')

    <div class="mb-4">
        <label for="titre" class="block font-semibold mb-1">Titre (optionnel)</label>
        <input type="text" name="titre" id="titre" value="{{ old('titre', $detail->titre) }}" class="w-full border px-3 py-2 rounded" />
        @error('titre')
    ...
    <div class="mb-4">
        ...
        <textarea name="contenu" id="contenu" class="w-full border rounded px-3 py-2" rows="6">{{ old('contenu', $detail->contenu) }}</textarea>
        @error('contenu')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="mb-4">
        ...
        <input type="number" name="ordre" id="ordre" value="{{ old('ordre', $detail->ordre) }}" min="0" class="w-24 border px-3 py-2 rounded" />
        @error('ordre')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
